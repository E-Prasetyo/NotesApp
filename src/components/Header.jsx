...
import useLocaleContext from '../hooks/useLocaleContext.jsx';
...
function Header({user, logoutHandler}) {
    ...
    return (    
        <header>
            <h1>
                <Link to={"/"}> {dictionary[localCtx.lang].titleApp}</Link>
            </h1>
            <nav className="navigation">
                {localCtx.accessToken != null && localCtx.accessToken != 'null' && (
                    <ul>
                        <li>
                            <Link to={"/archives"}>{dictionary[localCtx.lang].archiveMenu }</Link>
                        </li>
                    </ul>
                )} 
                <button
                    ...
                    type="button"
                    onClick={()=>localCtx.setLanguage(localCtx.lang)}
                >
                    <svg
                        stroke="currentColor"
                        fill="currentColor"
                ...
                <button 
                    ...
                    type="button"
                    onClick={()=>localCtx.setTheme()}
                >
                    {localCtx.theme == 'light' && (
                        <svg 
                            stroke="currentColor" 
                            fill="currentColor" 
                            strokeWidth="0" 
